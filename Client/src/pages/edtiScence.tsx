...
const GenerateVideoPage: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    updatedScenes[editingScene.index] = {
      ...editingScene,
      description: "mehulagarwal sips champagne from a crystal flute on the deck of a yacht",
  ...
  useEffect(() => {
    try {
      ...
      const mergedScenes = muxData.map((muxItem: any) => {
        const promptMatch = structuredPrompts.find((prompt: any) =>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Navbar />
      <h1 className="text-2xl font-semibold text-gray-900 text-center my-6">Generate your video</h1>
      ...
      <div className="w-full border border-gray-300 rounded-lg overflow-hidden">
        {/* Table Header */}
        <div className="grid grid-cols-5 gap-4 p-3 bg-gray-100 border-b text-gray-600 text-sm font-semibold">
          <span className="pl-3">Scene visual</span>
          <span>Scene description</span>
          <span>Dialogue</span>
          <span>Emotion</span>
          <span className="pr-3 text-right">Edit</span>
        </div>

        {/* Table Rows */}
        <div className="divide-y">
          {scenes.map((scene, index) => (
            <div key={index} className="grid grid-cols-5 gap-4 items-center p-4">
              {/* Image */}
              <img
                src={scene.image}
                alt="Scene"
              ...
              <p className="text-sm text-gray-900">
                {scene.dialogue || (
                  <span className="text-gray-500">Background music, no dialogue</span>
                )}
              </p>
              ...
              <div className="flex items-center space-x-2">
                ...
                ></span>
                <span className="text-sm font-medium text-gray-900">{scene.emotion}</span>
              </div>

              {/* Edit Button - Right-Aligned */}
              <div className="text-right">
                <button className="p-2 bg-transparent hover:bg-gray-200 rounded-full"
  ...
  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
    ...
    <div className="bg-black/30 backdrop-blur-xl rounded-t-3xl shadow-2xl p-6 w-full max-w-md relative border border-white/20 transform translate-y-0 animate-slide-up">
      ...
      {/* Modal Title */}
      <h3 className="text-xl font-semibold text-white text-center mb-4">What kind of scene do you want?</h3>
